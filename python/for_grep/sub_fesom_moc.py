...
def plot_xmoc(lat, depth, moc, bottom=[], which_moc='gmoc',
    ...
    cmap0,clevel = colormap_c2c(cmin,cmax,cref,cnumb,'PuOr')
    cbot = [0.5,0.5,0.5]
    do_drawedges=True
    print(clevel)
    ...
    CS=plt.contour(lat,depth,data_plot,levels=clevel,colors='k',linewidths=[0.5,0.25],antialiased=True)
...
#+___PLOT ACC OVERTRUNING CIRCULATION TIME-SERIES_______________________+
#|                                                                             |
#+_____________________________________________________________________________+
def plot_acc_tseries(time,moc_t,which_lat=['max'],which_moc='ACC',str_descript='',str_time='',figsize=[]):    
    import matplotlib.patheffects as path_effects
    from matplotlib.ticker import AutoMinorLocator
    ...
    for ii in range(len(which_lat)):
        if which_lat[ii]=='max':
            ...
            str_label='max {:s} at: {:2.1f}°N'.format(which_moc.upper(),which_lat[ii])
        hp=ax.plot(time,moc_t[:],\
                   linewidth=2,label=str_label,\
                   path_effects=[path_effects.SimpleLineShadow(offset=(1.5,-1.5),alpha=0.0),path_effects.Normal()])
        # plot mean value with trinagle 
        plt.plot(time[0]-(time[-1]-time[0])*0.015,moc_t[:].mean(),\
                 marker='<',markersize=8,markeredgecolor='k',markeredgewidth=0.5,\
                 color=hp[0].get_color(),zorder=3,clip_box=False,clip_on=False)
        
        # plot std. range
        plt.plot(time[0]-(time[-1]-time[0])*0.015,moc_t[:].mean()+moc_t[:].std(),\
                 marker='^',markersize=6,markeredgecolor='k',markeredgewidth=0.5,\
                 color=hp[0].get_color(),zorder=3,clip_box=False,clip_on=False)
    ...
    ax.xaxis.set_minor_locator(minor_locator)
    plt.grid(which='major')
    plt.xticks(np.arange(1990,2949,100))
    plt.xlim(time[0]-(time[-1]-time[0])*0.015,time[-1]+(time[-1]-time[0])*0.015)    
    plt.show(block=False)
